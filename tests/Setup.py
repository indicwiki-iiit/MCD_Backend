...
class Setup:
    ...
    def generate_user(self, fake_user, **kwargs):
        ...
        response = self.app.post('/api/user/', data=dumps(user), content_type='application/json')
    ...
    def generate_task(self, fake_task, **kwargs):
        ...
        response = self.app.post(f'/api/task/genric/?auth_token={self.auth_token}', data=task,
    ...
    def insert_response(self, task_id, qn_id, body):
        response = self.app.post(
            f"/api/task/{task_id}/annotation/{qn_id}/?auth_token={self.auth_token}",
